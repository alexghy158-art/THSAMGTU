(function () {
  ...
  function nickToEmail(nick) {
    const slug = nick
      ...
      .replace(/\.+/g, ".")
      .replace(/^\.|\.$/g, "") || "user";
    // латиница/цифры предпочтительнее для email; кириллицу кодируем
    const ascii = slug.replace(/[а-яё]/g, (ch) => "u" + ch.charCodeAt(0).toString(16));
    return ascii + "@tvoyhod.samgtu.local";
  }
